    ...
    public class EA : blockModel {
        ...
        public static void atualizarRV0(Deck deck, Semanas sAtual, Semanas sBase, DeckNW deckNW) {
            ...
            if (lstEA.Count != 0) {
                ...
                foreach (EA ea in deck.ea) {

                    var desloc = ((sAtual.ano - sBase.ano) * 12 + sAtual.mes) - sBase.mes;

                    for (int j = 12 - desloc; j >= 2; j--) {
                        PropertyInfo campoMesA = ea.GetType().GetProperty(String.Concat("campo", j.ToString()));
                        PropertyInfo campoMesB = ea.GetType().GetProperty(String.Concat("campo", (j + desloc).ToString()));
                        campoMesB.SetValue(ea, campoMesA.GetValue(ea, null).ToString(), null);
                    }
                    for (int j = 2; j < 2 + desloc; j++) {
                        PropertyInfo campoMesA = ea.GetType().GetProperty(String.Concat("campo", j.ToString()));
                        campoMesA.SetValue(ea, "xxxxxx", null);
                    }
