...
namespace DecompTools.ModelagemDC {
    public class Restricoes : blockModel {
        ...
        public static void atualizarRV0(Deck deck, int nSemanasAtual, int nSemanasBase, string bloco) {
            ...
            Type t = Type.GetType("DecompTools.ModelagemDC." + bloco);
        ...
        protected static void atualizarRV0Opcional(Deck deck, Deck deckBase, string bloco, Semanas s, Semanas sBase) {
            PropertyInfo blockRest = deck.GetType().GetProperty(bloco.ToLower());
            Type t = Type.GetType("DecompTools.ModelagemDC." + bloco);
            ...
            foreach (var rhGrp in blocoGrouped) {
                ...
                foreach (var rh in rhGrp.Where(r => bloco2.Contains(r.bloco)).OrderByDescending(r => r.campo2)) {
                    ...
                    if (estagio - offsetEstagios <= 1) {
                        rh.campo2 = rh.campo2.Replace(estagio.ToString(), "1");// "   1";
                        blockListNew.Add(rh);
                        break;
                    } else {
                        rh.campo2 = rh.campo2.Replace(estagio.ToString(), (estagio - offsetEstagios).ToString());//"   " + (estagio - offsetEstagios).ToString();
                        blockListNew.Add(rh);
                    }
                }
        ...
        public static void atualizarRVX(Deck deck, string bloco) {
            bool c;
            PropertyInfo blockRest = deck.GetType().GetProperty(bloco.ToLower());
            var prop = ((IList)blockRest.GetValue(deck, null));

            if (prop == null) return;
                 

            IList<Restricoes> lstRestricoes = prop.Cast<Restricoes>().ToList();
        ...
        public static int ordemBloco(string blocoNome) {
            ...
            blocos[12] = "FI"; blocos[13] = "FT"; blocos[14] = "FE";
