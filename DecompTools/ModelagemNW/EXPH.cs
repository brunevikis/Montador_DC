    ...
    public class EXPH : blockModel {
        ...
        public override void preencheCampos(string[] s) {
            try {
                ...
                this.Usina = s[2];
                this.Enchimento = s[3];
                if (s[4] == null) return; this.Duracao = String.Equals(s[4], String.Empty) ? 0 : int.Parse(s[4]);
                if (s[5] == null) return; this.Volume = String.Equals(s[5], String.Empty) ? 0 : double.Parse(s[5].Replace(".", ","));
                this.Entrada = s[6];
                if (s[7] == null) return; this.Pot = String.Equals(s[7], String.Empty) ? 0 : double.Parse(s[7].Replace(".", ","));
                if (s[8] == null) return; this.Maquina = String.Equals(s[8], String.Empty) ? 0 : int.Parse(s[8]);
                if (s[9] == null) return; this.Conjunto = String.Equals(s[9], String.Empty) ? 0 : int.Parse(s[9]);
