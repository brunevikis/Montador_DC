...
{
    public class C_ADIC : blockModelNW
    {
        ...
        public static void leArquivo(string caminho, DeckNW deck)
        {
            string Submercado = "";
            string Usina = "";
            List<C_ADIC> lst = new List<C_ADIC>();
            ...
            using (StreamReader objReader = new StreamReader(caminho))
            {
                ...
                while (!objReader.EndOfStream)
                {
                    ...
                    if (sLine != null && sLine != String.Empty && !sLine.Contains("XXX") && !sLine.Contains("POS") && !sLine.StartsWith(" 999"))
                    {
                        if (sLine.StartsWith("   "))
                        {
                            try
                            {
                                Submercado = sLine.Substring(4, 10).Trim();
                                Usina = sLine.Substring(14, 18).Trim();
                            }
                            catch (ArgumentOutOfRangeException)
                            {
                                Submercado = sLine.Trim();
                            }
                        }
                        else
                        {
